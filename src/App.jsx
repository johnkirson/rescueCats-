...
/**
 * v6.4.1 — STRICT hide threshold/rope until camera is enabled
 * -----------------------------------------------------------
 * ✅ Новое: явный флаг uiReady (камера включена + barY задан),
 *           всё рисуется только при uiReady=true.
 * ✅ Линия больше не появляется до Enable Camera.
 * ✅ Reset полностью очищает UI и сбрасывает barY.
 */
...
export default function App(){
  ...
  const [uiReady,setUiReady]=useState(false); // <-- ✅ Рисуем HUD только при true
  ...
  useEffect(()=>{ barYRef.current=barY; setUiReady(camReady && barY!=null); },[barY,camReady]);
  ...
  // Resize — barY не задаём до включения камеры
  useEffect(()=>{
    const resize=()=>{
      ...
      updateGeom();
      if(camReady && barYRef.current==null && uiRef.current){ const mid=Math.floor(uiRef.current.height*0.5); setBarY(mid); }
    };
  ...
  async function enableCamera(){
    ...
    try{
      ...
      setCamReady(true);
      if(uiRef.current){ const mid=Math.floor(uiRef.current.height*0.5); setBarY(mid); }
      spawnCatCentered();
      restartRAF();
  ...
  // Milestones/effects (same as v6.4)
  ...
  const tick = async ()=>{
    ...
    try {
      ...
      if(uiReady && recordingRef.current && detectorRef.current){
      ...
      // Effects + HUD (HUD можно рисовать всегда)
      drawEffectsLayer(u);
  ...
  function onPointerDown(e){ if(!uiReady) return; const y=getY(e); if(y==null) return; setBarY(y); if(catRef.current.mode==='idle') alignCatToBar(); draggingRef.current=true; }
  function onPointerMove(e){ if(!uiReady||!draggingRef.current) return; const y=getY(e); if(y==null) return; setBarY(y); if(catRef.current.mode==='idle') alignCatToBar(); }
  ...
  // Reset helper — полностью очищаем слой UI
  function hardClearUI(){ const u=uiRef.current; if(!u) return; const ctx=u.getContext('2d'); ctx.clearRect(0,0,u.width,u.height); }
    ...
    <div style={{position:'fixed',inset:0,background:'#000',color:'#fff',overflow:'hidden'}}>
      ...
      <div style={{position:'absolute',left:0,right:0,bottom:0,padding:'10px env(safe-area-inset-right) 14px env(safe-area-inset-left)',display:'grid',gap:8}}>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
          {!camReady ? (
            <button onClick={async()=>{ hardClearUI(); await enableCamera(); }} style={btn(1,'#22c55e')}>Enable Camera</button>
          ) : !recording ? (
          ...
          <button onClick={()=>{ savedRef.current=0; setSaved(0); repRef.current={phase:'down',lastAbove:0,lastRepAt:0}; seatedCatsRef.current=[]; firedMilestonesRef.current=new Set(); effectsRef.current={ fireworks:[], confetti:[], overlay:{active:false,label:'',until:0}, seatedStyle:{mode:'default',until:0} }; setBarY(null); setCamReady(false); setUiReady(false); hardClearUI(); setMsg('Enable camera to start'); }} style={btn()}>Reset</button>
        </div>
