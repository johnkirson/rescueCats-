...
/**
 * v4.2 — Camera default/mirror fix + stable switch
 * - No more initial "front flicker": we unlock labels then STOP that stream before attaching.
 * - Default really picks Ultra‑Wide → Wide → Front. UI shows the actual active bucket.
 * - Mirroring is derived from the ACTIVE stream's facingMode/label (front=user ⇒ mirror, back=environment ⇒ not).
 * - Switching buckets replaces stream reliably and updates geometry + flipHorizontal, fixing Ultra‑Wide mirroring.
 * - Keeps: cats sit at bottom, rope HUD, recording, pose overlay.
 */
...
export default function PullUpRescueV42(){
  ...
  const [initializingCam,setInitializingCam]=useState(false);
  ...
  function toBuckets(devices){ const vids=devices.filter(d=>d.kind==='videoinput'); const pick=(fn)=>vids.find(d=>fn(d.label||'')); const ultra = vids.find(d=>isUltraLabel(d.label||'')); const fronts=vids.filter(d=>isFrontLabel(d.label||'')); const backs = vids.filter(d=>!isFrontLabel(d.label||'')); const wide  = backs.find(d=>!isTeleLabel(d.label||'')) || backs[0] || null; return { ultra: ultra?.deviceId||null, wide: wide?.deviceId||null, front: fronts[0]?.deviceId||null }; }

  async function enableCamera(){ setDebug(""); setInitializingCam(true); try{ await ensureDetector(); // 1) unlock labels, then stop immediately (avoid front flicker)
      const pre=await navigator.mediaDevices.getUserMedia({video:true,audio:false});
      const devices=await navigator.mediaDevices.enumerateDevices();
      pre.getTracks().forEach(t=>t.stop()); // do not attach this stream
      const buckets=toBuckets(devices); setBucketMap(buckets);
      const preferred = buckets.ultra ? 'ultra' : (buckets.wide ? 'wide' : 'front');
      const okBucket = await switchToBucket(preferred);
      setBucketChoice(okBucket); // reflect actual active
      setCamReady(true);
      updateGeom();
      cancelAnimationFrame(rafRef.current||0); rafRef.current=requestAnimationFrame(tick);
      if (uiRef.current && barYRef.current==null) { const mid=Math.floor(uiRef.current.height*0.5); setBarY(mid); }
    ...
    finally{ setInitializingCam(false); }
  }
  ...
  async function switchToBucket(bucket){ try{ const deviceId=bucketMap[bucket]; const baseConstraints = { width:{ideal:1920}, height:{ideal:1080}, frameRate:{ideal:30, max:60} };
      ...
      if(!stream){ // fallback try back wide then front
        try{ stream=await navigator.mediaDevices.getUserMedia({ video: baseConstraints, audio:false }); }catch{ stream=await navigator.mediaDevices.getUserMedia({ video:true, audio:false }); }
      }
      ...
      // set bucket by verification: derive from stream label
      const label = stream.getVideoTracks?.()[0]?.label || '';
      const finalBucket = isFrontLabel(label) ? 'front' : (isUltraLabel(label) ? 'ultra' : 'wide');
  ...
  // --- Rope & Cats (same visual logic as v4.1) ---
    ...
    <div style={{position:'fixed',inset:0,background:'#000',color:'#fff',overflow:'hidden'}}>
      ...
      <div style={{position:'absolute',top:0,left:0,right:0,padding:'10px env(safe-area-inset-right) 10px env(safe-area-inset-left)',display:'flex',justifyContent:'space-between',alignItems:'center',gap:8}}>
        ...
        <div style={{display:'flex',gap:8,alignItems:'center'}}>
          {camReady && (
            <select disabled={initializingCam} value={bucketChoice} onChange={async(e)=>{ const b=e.target.value; await switchToBucket(b); }} style={{background:'rgba(255,255,255,.12)',color:'#fff',border:0,borderRadius:10,padding:'6px'}}>
          ...
          {!detecting ? (
            <button disabled={!camReady||initializingCam} onClick={()=>{ if(!camReady) return setMsg('Enable camera first'); setDetecting(true); detectingRef.current=true; setMsg('Saving cats…'); }} style={btn(camReady&&!initializingCam?1:.5)}>Start</button>
          ) : (
