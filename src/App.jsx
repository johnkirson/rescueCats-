...
/**
 * v3.3 — FIX: MoveNet architecture string
 * Error was: "Invalid architecture Lightning. Should be one of SinglePose.Lightning,SinglePose.Thunder,MultiPose.Lightning".
 * We now use the official enum: poseDetection.movenet.modelType.SINGLEPOSE_LIGHTNING.
 * If that fails on a given build, we fallback to Thunder → MultiPose Lightning.
 * (Keeps all iOS camera fixes from v3.2.)
 */
...
export default function PullUpRescueV33(){
  ...
  async function createMoveNetDetector(){
    ...
    await tf.ready();
    // Try SinglePose.Lightning first (fast), then Thunder (more accurate), then MultiPose.Lightning
    const opts = [
    ...
    for (const o of opts){
      try {
        return await poseDetection.createDetector(poseDetection.SupportedModels.MoveNet, o);
      } catch(e){ lastErr = e; }
    }
    throw lastErr;
  }
  ...
  async function enableCamera(){
    ...
    try{
      ...
      // 1) Ask for generic camera to unlock labels on iOS
      ...
      // 2) enumerate and choose back/front by label → deviceId
      const devices = await navigator.mediaDevices.enumerateDevices();
      const videos = devices.filter(d=>d.kind==='videoinput');
      const isBack = (d)=>/back|rear|environment/i.test(d.label);
      const isFront = (d)=>/front|user|face/i.test(d.label);
      let target = useBack ? (videos.find(isBack) || videos.find(d=>!isFront(d)) || videos[0]) : (videos.find(isFront) || videos[0]);
      let stream = pre;
      if(target?.deviceId){
        try { stream = await navigator.mediaDevices.getUserMedia({ video: { deviceId: { exact: target.deviceId } }, audio:false }); }
        catch(e){ setDebug(`Exact deviceId failed: ${e.name}`); }
      }
