...
/**
 * v5.8 — Build-fix + polish
 * - Fixes syntax issues that caused "Unexpected end of file" during build.
 * - Keeps: attached-follow (above+below), delayed drop (time+depth), landing sprite (7),
 *          counter via ref, HUD, rope scale XY, iOS-friendly recording.
 */

// ===================== CALIBRATION =====================
// ——— Visual alignment ———
export const ROPE_BASELINE_FROM_BOTTOM = 0.30; // chosen by you
export const CAT_BASELINE_ABOVE_ROPE_PX = 0;    // chosen by you
export const DEFAULT_SENSITIVITY = 27;          // chosen by you
export const INFER_EVERY_MS = 70;               // pose cadence (~14Hz)
...
// ——— Rope scaling (X/Y) ———
export const ROPE_SCALE_X = 1.2; // width relative to screen width
export const ROPE_SCALE_Y = 1.0; // vertical stretch of entire fire.png

// ——— Delayed drop ———
export const DROP_TRAVEL_BELOW_PX = 22; // px to carry below threshold
export const DROP_MIN_TIME_MS = 500;    // ms held below threshold
...
const MOVENET_EDGES = {
  0: [0, 1], 1: [1, 3], 2: [0, 2], 3: [2, 4], 4: [5, 7], 5: [7, 9], 6: [6, 8],
  7: [8, 10], 8: [5, 6], 9: [5, 11], 10: [6, 12], 11: [11, 12], 12: [11, 13],
  13: [13, 15], 14: [12, 14], 15: [14, 16]
};
...
export default function PullUpRescueV58() {
  ...
  const inferMapRef = useRef({ s: 1, ix: 0, iy: 0 });
  ...
  const [msg, setMsg] = useState('Drag the rope to the bar height');
  const [debug, setDebug] = useState('');
  const [recording, setRecording] = useState(false);
  const recordingRef = useRef(false);
  const [canRecord, setCanRecord] = useState(false);
  const [barY, setBarY] = useState(null);
  const barYRef = useRef(null);
  ...
  const catRef = useRef({
    mode: 'idle', x: 0, y: 0, vx: 0, vy: 0, lastT: 0,
    attachDy: 0, belowStart: 0, maxDepthBelow: 0, landUntil: 0
  });
  ...
  useEffect(() => {
    ...
    entries.forEach(([key, src]) => {
      const im = new Image();
      im.onload = () => { loaded[key] = im; left--; if (left === 0) setImgs(loaded); };
      im.src = src;
    });
  }, []);
  ...
  function updateGeom() {
    ...
    const inW = 320; const inH = Math.round(inW * vh / vw) || 240;
    const c = inferCanvasRef.current; c.width = inW; c.height = inH;
  }

  function setMirrorFromStream(stream) {
    try {
      const track = stream.getVideoTracks?.()[0];
      const s = track?.getSettings?.() || {}; const label = track?.label || '';
  ...
  async function createMoveNetDetector() {
    ...
    let lastErr;
    for (const o of opts) {
      try { return await poseDetection.createDetector(poseDetection.SupportedModels.MoveNet, o); }
      catch (e) { lastErr = e; }
    }
    throw lastErr;
  }
  ...
  async function ensureDetector() {
    if (!detectorRef.current) detectorRef.current = await createMoveNetDetector();
  }
  ...
  async function enableCamera() {
    ...
    try {
      ...
      const buckets = toBuckets(devices); setBucketMap(buckets);
      const preferred = buckets.ultra ? 'ultra' : (buckets.wide ? 'wide' : 'front');
      await switchToBucket(preferred);
      setCamReady(true);
      cancelAnimationFrame(rafRef.current || 0);
      rafRef.current = requestAnimationFrame(tick);
    } catch (e) {
      console.error(e); setMsg('Camera init failed.'); setDebug(`${e.name || 'Error'}: ${e.message || e}`);
    }
  }

  async function switchToBucket(bucket) {
    try {
      const deviceId = bucketMap[bucket];
      const baseConstraints = { width: { ideal: 1920 }, height: { ideal: 1080 }, frameRate: { ideal: 30, max: 60 } };
      let stream;
      if (deviceId) {
        try {
          stream = await navigator.mediaDevices.getUserMedia({ video: { deviceId: { exact: deviceId }, ...baseConstraints }, audio: false });
        } catch (e) { setDebug(`Exact ${bucket} failed: ${e.name}`); }
      }
      if (!stream) {
        try { stream = await navigator.mediaDevices.getUserMedia({ video: baseConstraints, audio: false }); }
      ...
      setBucketChoice(finalBucket); return finalBucket;
  ...
  const tick = async () => {
    ...
    if (recordingRef.current && detectorRef.current) {
      if (now - lastInferRef.current >= INFER_EVERY_MS) {
        ...
        const s = Math.max(inC.width / vw, inC.height / vh);
        const iw = vw * s, ih = vh * s;
        const ix = (inC.width - iw) / 2, iy = (inC.height - ih) / 2;
        inferMapRef.current = { s, ix, iy };
        ic.clearRect(0, 0, inC.width, inC.height);
        ic.drawImage(video, ix, iy, iw, ih);
        try {
      ...
      if (pose) {
        ...
        const { s, ix, iy } = inferMapRef.current;
        const mapped = kps.map((kp) => {
          const xv = (kp.x - ix) / s; const yv = (kp.y - iy) / s;
          const xd = dx + (mirrored ? (vw - xv) : xv) * scale;
          const yd = dy + yv * scale;
          return { X: xd, Y: yd, score: kp.score, raw: { xv, yv } };
        });

        if (showPose) { u.save(); u.strokeStyle = 'rgba(255,255,255,.9)'; u.lineWidth = 2; drawPoseMapped(u, mapped); u.restore(); }

        const nose = mapped[0];
        const by = barYRef.current; const sens = sensitivityRef.current;
  ...
  function drawPoseMapped(ctx, kps) {
    ...
    for (const k of kps) {
      if (k.score > 0.3) { ctx.beginPath(); ctx.arc(k.X, k.Y, 3, 0, Math.PI * 2); ctx.fillStyle = 'rgba(255,255,255,.95)'; ctx.fill(); }
    }
  }
  ...
  function spawnCatCentered() {
    const u = uiRef.current; if (!u) return; const p = window.devicePixelRatio || 1;
    const W = u.width; const y = barYRef.current ?? Math.floor(u.height * 0.5);
  ...
  function startCatFall() {
    const now = performance.now(); const c = catRef.current;
    c.mode = 'falling'; c.vx = (Math.random() * 2 - 1) * 24; c.vy = 0; c.lastT = now; c.belowStart = 0; c.maxDepthBelow = 0;
  }
  ...
  function drawSeatedCats(ctx, imgs) {
    ...
    for (const s of seatedCatsRef.current) { const yNudge = (CAT_Y_NUDGE_PX.seated || 0) * (window.devicePixelRatio || 1); ctx.drawImage(im, Math.round(s.x - w / 2), Math.round(s.y - h + yNudge), w, h); }
  }
  function drawActiveCat(ctx, imgs) {
    const c = catRef.current; stepActiveCat(); if (!c) return; if (c.mode === 'seated') return; // do not draw idle flicker
    const state = (c.mode === 'attached') ? 'attached' : (c.mode === 'falling') ? 'falling' : (c.mode === 'landing') ? 'landing' : 'idle';
    const im = state === 'attached' ? imgs.cat_attached : state === 'falling' ? imgs.cat_jump : state === 'landing' ? imgs.cat_land : imgs.cat_idle;
    if (!im) return; const w = catWidthPx(state); const h = catHeightFor(im, w); const yNudge = (CAT_Y_NUDGE_PX[state] || 0) * (window.devicePixelRatio || 1);
  ...
  // interactions: drag to set rope height
  ...
  function startRecording() {
    ...
    const r = rec.getContext('2d'); rec.width = base.width; rec.height = base.height;
    const mime = pickMime(); if (!mime) return;
    recordedChunksRef.current = []; setRecording(true); recordingRef.current = true;
    const targetFps = 30;
    const compose = () => { if (!recordingRef.current) return; r.clearRect(0, 0, rec.width, rec.height); r.drawImage(base, 0, 0); r.drawImage(ui, 0, 0); requestAnimationFrame(compose); };
    requestAnimationFrame(compose);
    const stream = rec.captureStream(targetFps);
    let mr;
    try { mr = new MediaRecorder(stream, { mimeType: mime, videoBitsPerSecond: 6_000_000 }); }
    catch { mr = new MediaRecorder(stream, { mimeType: mime }); }
    mr.ondataavailable = (e) => { if (e.data && e.data.size > 0) recordedChunksRef.current.push(e.data); };
    mr.onstop = () => {
      const type = mr.mimeType || mime; const blob = new Blob(recordedChunksRef.current, { type });
      if (!blob || blob.size < 150000) { setMsg('Recording tiny — iOS codec limited. Try again or use iOS Screen Recording.'); return; }
      const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url;
      const ts = new Date().toISOString().replace(/[:.]/g, '-'); a.download = `pullup-rescue-${ts}.${type.includes('mp4') ? 'mp4' : 'webm'}`; a.click(); setMsg('Recording saved');
    };
    ...
    <div style={{ position: 'fixed', inset: 0, background: '#000', color: '#fff', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, padding: '10px env(safe-area-inset-right) 14px env(safe-area-inset-left)', display: 'grid', gap: 8 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          ...
          <button onClick={() => { savedRef.current = 0; setSaved(0); repRef.current = { phase: 'down', lastAbove: 0, lastRepAt: 0 }; seatedCatsRef.current = []; spawnCatCentered(); }} style={btn()}>Reset</button>
        </div>
