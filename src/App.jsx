...
/**
 * v6.1 — v5.8 parity + Milestones
 * --------------------------------------------------
 * ✔ Сохраняет поведение v5.8 (камера, выбор камер, запись, поза, follow above+below,
 *    delayed drop, landing→seated без idle-флика, HUD, rope scale X/Y).
 * ✨ Добавляет milestones-движок (конфиг, эффекты: fireworks/confetti/group overlay,
 *    временная анимация сидящих котов). Всё опционально и легко расширяемо.
 */

// ===================== CALIBRATION =====================
// ——— Visual alignment ———
export const ROPE_BASELINE_FROM_BOTTOM = 0.30; // где базовая линия троса внутри fire.png (0..1 от низа)
export const CAT_BASELINE_ABOVE_ROPE_PX = 0;   // смещение лап кота над базовой линией троса (px устройства)
export const DEFAULT_SENSITIVITY = 27;         // порог от троса (px) — линия срабатывания
export const INFER_EVERY_MS = 70;              // частота инференса (~14Hz)
...
// ——— Rope scaling (X/Y) ———
export const ROPE_SCALE_X = 1.2;  // (v5.8) ширина fire.png относительно ширины экрана
export const ROPE_SCALE_Y = 1.0;  // вертикальное растяжение fire.png

// ——— Delayed drop ———
export const DROP_TRAVEL_BELOW_PX = 22; // мин. глубина ниже порога, прежде чем отпустить
export const DROP_MIN_TIME_MS = 500;    // мин. время ниже порога, прежде чем отпустить

// ——— Landing ———
export const CAT_LAND_DURATION_MS = 220; // сколько показывать спрайт посадки (7.png)

// ——— Milestones (декларативно) ———
/** Доступные действия:
 *  - seatedSwap: временная анимация сидящих котов (dance/chant) без ассетов
 *  - fireworks: фейерверк-частицы сверху
 *  - confetti: конфетти сверху
 *  - groupOverlay: оверлей-плашка (можно заменить PNG в будущем)
 */
...
export default function PullUpRescueV61(){
  // ===== Refs & state (как в v5.8) =====
  ...
  const [imgs,setImgs]=useState({}); useEffect(()=>{ const entries=Object.entries(SPRITES); const loaded={}; let left=entries.length; entries.forEach(([k,src])=>{ const im=new Image(); im.onload=()=>{ loaded[k]=im; left--; if(left===0) setImgs(loaded); }; im.src=src; }); },[]);
  ...
  function runMilestone(rule){ const now=performance.now(); const fx=effectsRef.current; switch(rule.action){
  ...
  function stepAndDrawEffects(ctx){ const now=performance.now(); const fx=effectsRef.current; const u=uiRef.current; if(!u) return; const W=u.width,H=u.height; ctx.save();
    // fireworks
  ...
  // ===== Tick (как в v5.8, + эффекты/милстоуны) =====
  const tick = async ()=>{ const video=videoRef.current, base=baseRef.current, ui=uiRef.current; if(!video||!base||!ui) return; const b=base.getContext('2d'); const u=ui.getContext('2d'); const {W,H,vw,vh,scale,dx,dy,mirrored}=geomRef.current; const inC=inferCanvasRef.current; if(!W) updateGeom();
    ...
    // инференс только при записи (как в v5.8)
      ...
      const pose = lastPoseRef.current; if(pose){ const kps=pose.keypoints; const {s,ix,iy}=inferMapRef.current; const mapped=kps.map((kp)=>{ const xv=(kp.x-ix)/s; const yv=(kp.y-iy)/s; const xd = dx + (mirrored ? (vw - xv) : xv) * scale; const yd = dy + yv * scale; return {X:xd,Y:yd,score:kp.score, raw:{xv,yv}}; });
        ...
        const nose=mapped[0]; const by=barYRef.current; const sens=sensitivityRef.current; if(nose?.score>0.4 && by!==null){ const thr=by - sens; const above = nose.Y <= thr; const {next}=updateRepState(repRef.current,above,now); repRef.current=next;
          ...
          if(catRef.current.mode==='attached'){
            ...
            else { if(!catRef.current.belowStart){ catRef.current.belowStart=now; catRef.current.maxDepthBelow=0; } const depth=Math.max(0, nose.Y - thr); if(depth>catRef.current.maxDepthBelow) catRef.current.maxDepthBelow=depth; const timeEnough=(now-catRef.current.belowStart)>=DROP_MIN_TIME_MS; const travelEnough=catRef.current.maxDepthBelow>=DROP_TRAVEL_BELOW_PX; if(timeEnough && travelEnough){ startCatFall(); } }
          }
        }
      }
    }
    ...
    <div style={{position:'fixed',inset:0,background:'#000',color:'#fff',overflow:'hidden'}}>
      ...
      <div style={{position:'absolute',left:0,right:0,bottom:0,padding:'10px env(safe-area-inset-right) 14px env(safe-area-inset-left)',display:'grid',gap:8}}>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
          ...
          <button onClick={()=>{ savedRef.current=0; setSaved(0); repRef.current={phase:'down',lastAbove:0,lastRepAt:0}; seatedCatsRef.current=[]; firedMilestonesRef.current=new Set(); effectsRef.current={ fireworks:[], confetti:[], overlay:{active:false,label:'',until:0}, seatedStyle:{mode:'default',until:0} }; spawnCatCentered(); }} style={btn()}>Reset</button>
        </div>
